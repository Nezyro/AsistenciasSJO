...
$nombre = $dia = $hora = $id_profesor = $fecha_inicio = $fecha_final = "";
$nombre_err = $dia_err = $hora_err = $id_profesor_err = $fecha_inicio_err = $fecha_final_err = $imagen_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["fecha_inicio"])) {
        $fecha_inicio_err = "Por favor, introduzca una fecha de inicio.";
    } else {
        $fecha_inicio = $_POST["fecha_inicio"];
    }

    if (empty($_POST["fecha_final"])) {
        $fecha_final_err = "Por favor, introduzca una fecha final.";
    } else {
    ...
    if (empty($nombre_err) && empty($dia_err) && empty($hora_err) && empty($id_profesor_err) && empty($fecha_inicio_err) && empty($fecha_final_err) && empty($imagen_err)) {
        ...
        if ($stmt = $db->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                $clases_json_path = 'clases.json';
                $clases_json = file_exists($clases_json_path) ? json_decode(file_get_contents($clases_json_path), true) : ["clases" => []];
                ...
                $clases_json["clases"][] = $new_clase;

                file_put_contents($clases_json_path, json_encode($clases_json, JSON_PRETTY_PRINT));
...
<div class="wrapper">
    ...
    <form action="register_materias.php" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <span class="text-danger"><?php echo $fecha_final_err; ?></span>
        </div>
        <div class="form-group">
